...
import { MapPin, Info, Beer as BeerIcon, Compass, Phone, Globe } from 'lucide-react';
...
export function InteractiveMapContent({ breweries }: InteractiveMapContentProps) {
  ...
  const visibleBreweries = selectedRegion === 'All'
    ? breweries
    : breweries.filter(b => b.region === selectedRegion);
  ...
  const handleRegionClick = (region: MarylandRegion | 'All') => {
    ...
    const firstOfRegion = region === 'All'
      ? breweries[0]
      : breweries.find(b => b.region === region);
